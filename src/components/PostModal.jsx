...
const PostModal = ({
  ...
  const handleConfirm = async () => {
    try {
      const response = await api.put(
        `/posts/status/${postId}?status=${confirmModal.status}`
      );
  ...
  return (
    <div className="fixed bg-neutral-800/50 top-0 left-0 right-0 z-50 flex items-center justify-center w-full p-4 h-screen overflow-y-auto">
      <div className="relative w-full max-w-4xl max-h-full">
        <div className="relative bg-white rounded-lg shadow-sm dark:bg-gray-700">
          ...
          <div className="flex items-center p-4 md:p-5 space-x-3 border-t border-gray-200 rounded-b dark:border-gray-600">
            ...
            <button
              onClick={() =>
                setConfirmModal({ isOpen: true, status: "REJECTED" })
              }
              type="button"
      ...
      <ConfirmDelete
        ...
        message={`Are you sure you want to ${
          confirmModal.status === "PUBLISHED" ? "publish" : "reject"
        } this post?`}
